...
public class Locations extends MySQL {
     ...
     * @param l_time Timestamp representing the time at which the position was
     * recorded.
    ...
    /**
     * Retrieves the timestamps of the recorded positions between two moments in
     * time..
     *
     * @param i_time Timestamp of the initial time.
     * @param f_time Timestamp of the final time.
    ...
    public ArrayList<Pair<Point2D.Double, Long>> getFullLocation(long i_time, long f_time) {
        ...
        try {
            String query = "SELECT latitude, longitude,l_time FROM locations WHERE l_time >= ? and l_time <= ? ";
            st1 = con.prepareStatement(query);
            st1.setLong(1, i_time);
            st1.setLong(2, f_time);
            rs = st1.executeQuery();
            ...
            while (rs.next()) {
                arrayList.add(new Pair(new Point2D.Double(rs.getDouble("latitude"), rs.getDouble("longitude")), rs.getLong("l_time")));
            }
